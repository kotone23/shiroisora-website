...
export default async function LivePage() {
  ...
  const now = new Date();
  const futureLives = contents.filter(content => new Date(content.date) > new Date());

  return (
    <div className="container max-w-screen-lg mx-auto px-4">
      <h1 className="flex justify-center text-4xl font-bold mt-8 mb-8">Live</h1>
      {futureLives.length > 0 ? contents.filter(content => new Date(content.date) > now).map((content) => (
        <LiveEntry 
          key={content.sys.id} 
          title={content.title} 
          venue={content.venue} 
      ...
      {contents.filter(content => new Date(content.date) < now).map((content) => (
        <LiveEntry 
          key={content.sys.id} 
          title={content.title} 
          venue={content.venue} 
